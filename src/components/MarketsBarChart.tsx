...
import { marketBarTheme } from '../features/dashboard/marketChartTheme'
...
function MarketsTooltip({
  ...
  return (
    <div
      ...
      style={{ '--tip-accent': theme.glow } as CSSProperties}
    >
  ...
  return (
    <defs>
      <linearGradient id="market-grid-glow" x1="0" y1="1" x2="0" y2="0">
        <stop offset="0%" stopColor="hsl(62 96% 58% / 0.14)" />
        <stop offset="55%" stopColor="hsl(272 72% 62% / 0.08)" />
        ...
        return (
          <linearGradient key={row.index} id={gid} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={t.top}>
              <animate attributeName="stop-color" values={`${t.top};${t.mid};${t.top}`} dur="5s" repeatCount="indefinite" />
            </stop>
            <stop offset="38%" stopColor={t.mid} stopOpacity={0.95} />
            <stop offset="72%" stopColor={t.deep} stopOpacity={0.85} />
            <stop offset="100%" stopColor={t.base} stopOpacity={0.55} />
          </linearGradient>
        )
      })}
    </defs>
  ...
  return (
    <div className="markets-chart">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={rows} barCategoryGap="20%" margin={{ top: 10, right: 6, left: 2, bottom: 6 }}>
          ...
          />
          <Tooltip
            cursor={{ fill: 'hsl(62 96% 58% / 0.08)', stroke: 'hsl(272 72% 62% / 0.22)', strokeWidth: 1 }}
          ...
          >
            {rows.map((row) => (
              <Cell
                ...
                style={{ filter: `drop-shadow(0 10px 18px ${marketBarTheme(row.index).glow}50)` }}
              />
            ))}
          </Bar>
        </BarChart>
