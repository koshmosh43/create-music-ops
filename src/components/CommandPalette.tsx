...
export function CommandPalette({
  ...
  const actions = useMemo<Action[]>(() => {
    const list: Action[] = [
      { id: 'tab-ops', label: 'Go to Operations', hint: 'Navigate', icon: SlidersHorizontal, color: '#67e8f9', onSelect: () => run(() => onSelectTab('operations')) },
      { id: 'tab-mkt', label: 'Go to Markets', hint: 'Navigate', icon: SlidersHorizontal, color: '#a78bfa', onSelect: () => run(() => onSelectTab('markets')) },
      ...(['All', 'Spotify', 'Apple Music', 'YouTube', 'TikTok', 'Amazon'] as DspFilter[]).map((d) => ({
        id: `dsp-${d}`,
        label: d === 'All' ? 'All DSPs' : d,
        hint: 'Filter',
        icon: Disc3,
        color: d === 'All' ? '#67e8f9' : DSP_COLORS[d],
  ...
  return (
    <div className="fixed inset-0 z-50" onKeyDown={handleKey}>
      ...
      <div
        ...
        style={{ animation: 'cmd-scale-in .18s cubic-bezier(.16,1,.3,1)', boxShadow: '0 0 80px rgba(103,232,249,.08), 0 24px 60px rgba(0,0,0,.5)' }}
      >
        {/* search */}
        <div className="relative flex items-center gap-3 px-5 py-4">
          ...
          <Search size={20} className="shrink-0 text-cyan-400" />
          <input
            ref={inputRef}
            value={query}
            ...
            <li key={a.id}>
              ...
              >
                <span
                  ...
                  style={{
                    background: i === active ? `${a.color ?? '#67e8f9'}18` : 'rgba(255,255,255,.04)',
                    boxShadow: i === active ? `0 0 12px ${a.color ?? '#67e8f9'}25` : 'none',
                  }}
                >
                ...
                <span
                  ...
                  style={{ background: `${a.color ?? '#67e8f9'}15`, color: a.color ?? '#94a3b8' }}
                >
                  {a.hint}
                </span>
              </button>
...
export function CmdKHint() {
  ...
  return (
    <button
      onClick={openCmdk}
      className="group relative inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-2.5 text-sm text-slate-300 transition-all hover:border-cyan-300/30 hover:bg-white/[0.1] hover:text-white hover:shadow-[0_0_24px_rgba(103,232,249,.15)]"
    >
      <Search size={15} className="text-slate-500 transition-colors group-hover:text-cyan-300" />
      <span className="hidden sm:inline">Quick actions</span>
      <kbd className="ml-1 inline-flex items-center gap-0.5 rounded-lg border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[11px] text-slate-500 transition-colors group-hover:border-cyan-300/20 group-hover:text-cyan-300">
        <Command size={10} />K
      </kbd>
    </button>
  )
